...
module.exports.invoke = async function githubProvider(since, opts) {
	...
	const unarchivedReposPromises = repos.map(repo => {
		if (!repo.archived) return repo;

		const resPromise = octokit.graphql(`query repoArchivedDate($owner: String!, $repo: String!) {
			repository(owner: $owner, name: $repo) {
				archivedAt
			}
		}`, { owner: repo.owner.login, repo: repo.name });
		return resPromise.then(res => {
			const archivedAt = res.repository.archivedAt;
			if (archivedAt && new Date(archivedAt) > since) return repo;
		});
	});
	// Filter out undefined values
	repos = (await Promise.all(unarchivedReposPromises)).filter(repo => repo);

	const originals = repos.filter(r => !r.fork);
	const forks = repos.filter(r => r.fork);
	// TODO more aggressive forks processing
	return originals.map(repo => {
		const suffix = new Date(repo.created_at) < since ? ` (partial, since ${since.toDateString()})` : '';
		return `Project: ${repo.html_url}${suffix}`;
	}).concat(forks.map(repo => `Contributions: ${repo.html_url}`));
};
